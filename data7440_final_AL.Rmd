...
```{r}
as.data.frame(apipop_train1) %>% select(-awards)
```
...
components_iml <- Predictor$new(
  model = rf.model,
  data = as.data.frame(apipop_train1) %>% select(-awards),
  y = as.data.frame(apipop_train1) %>% pull(awards)
)
```

```{r message=FALSE, warning=FALSE, paged.print=FALSE}
if(file.exists("./data/models/interact") & train==FALSE) {
  print("Loading interact")
  load("./data/models/interact")
} else {
  print("Training interact")
  interact <- Interaction$new(components_iml)
  save(interact, file = "./data/models/interact")
}
```

```{r}
interact$results %>% arrange(desc(.interaction)) %>% 
  head
```

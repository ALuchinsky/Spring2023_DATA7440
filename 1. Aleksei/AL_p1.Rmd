...
Here is the list of 4 most important vriables for the Rule Fit model:
...
Here is the list of most impornant rules:

```{r}
rule.fit.imps$baseimps %>% head(4)
```
...
Again, we have two identical sets of the statistics, here is ordered plot of one of them:
...
```{r}
rule.fit.imps$baseimps %>% filter(grepl("stype", description) & grepl("ell", description)) %>% tail(1)
```
